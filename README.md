...
- якщо батч падає по timeout, клієнт автоматично ділить його на менші частини і повторює відправку;
- для `append_rows` можна задати окремий збільшений таймаут (`WEB_APP_APPEND_TIMEOUT`).

Що спробувати:
1. Збільшити `WEB_APP_APPEND_TIMEOUT` (наприклад, до `60`–`120`).
2. За потреби зменшити `WEB_APP_APPEND_BATCH_SIZE` (наприклад, до `100` або `50`) для ще стабільнішої роботи.
