...
- `ImportError: cannot import name "read_secret" from "config"`
  - зазвичай це побічна помилка, коли `config.py` падає під час імпорту (наприклад, невалідний JSON у `google.service_account_info`).
  - тепер невалідний JSON ігнорується (береться `None`), тож застосунок стартує в режимі Web App або fallback.

- `OSError: [Errno 28] inotify watch limit reached` у Streamlit
  - це обмеження файлових watcher-ів у Linux-середовищі, не помилка бізнес-логіки застосунку.
  - запуск: `streamlit run app.py --server.fileWatcherType none`
